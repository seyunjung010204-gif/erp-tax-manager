...
async function getAuthUser(req: NextRequest) {
  ...
  if (error || !data.user) throw new Error("인증 정보를 확인할 수 없습니다.");
...
async function buildMergedPdf(
  ...
  for (const record of records) {
    ...
    for (const file of linked) {
      ...
      if (contentType.includes("pdf") || file.file_name.toLowerCase().endsWith(".pdf")) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: attachments, error: attError } = await supabase
      .from("attachments")
      .select("*")
      .in("tax_record_id", recordIds.length ? recordIds : ["00000000-0000-0000-0000-000000000000"]);

    if (attError) throw attError;

    const typedAttachments = (attachments ?? []) as EvidenceAttachment[];
    ...
    if (mode === "zip" || mode === "pdf-separate") {
      for (const record of typedRecords) {
        const linked = typedAttachments.filter((a) => a.tax_record_id === record.id);
        ...
        for (const file of linked) {
          ...
          const name = `${record.record_date}_${safeFileName(
            record.vendor_name
          )}_${record.type === "purchase" ? "매입" : "매출"}_${record.supply_amount}_${safeFileName(
